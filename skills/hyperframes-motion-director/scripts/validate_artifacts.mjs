...
const checks = [
  ...
  },
  {
    ...
    pattern: /## Writing Standard[\s\S]*Sentence style:[\s\S]*Avoid:[\s\S]*Keep:[\s\S]*## Output[\s\S]*## Validation[\s\S]*first-frame dominant visual mass[\s\S]*hero-frame edge clearance[\s\S]*rendered MP4 still spot-check[\s\S]*visual asset floor[\s\S]*empty image asset directory check[\s\S]*pure-code exception approval[\s\S]*first eye target[\s\S]*transition midpoints[\s\S]*layout overflow[\s\S]*kinetic typography[\s\S]*SVG \/ CSS3 motion structure[\s\S]*anti-PPT verdict[\s\S]*## Kinetic Relay Score[\s\S]*First-eye impact \/ 20[\s\S]*Relay continuity \/ 20[\s\S]*Total \/ 100[\s\S]*## Style Gate[\s\S]*Background image or pure-code exception is verified:[\s\S]*Visual asset floor is met for non-typography videos:[\s\S]*Local background \/ screenshot \/ generated proof assets exist when needed:[\s\S]*Empty `assets\/images` is blocked unless pure-code exception is explicitly approved:[\s\S]*Snapshots show more than black background plus title\/cards:[\s\S]*Support asset decision is documented:[\s\S]*Support assets have role, source, motion purpose, safe zones, and deletion triggers:[\s\S]*Layout contract matches the image and message shape:[\s\S]*First-frame dominant visual mass is optically centered or intentionally offset:[\s\S]*Hero-frame product cards, screenshots, symbols, and CTA clear edge-safe bounds:[\s\S]*Rendered MP4 stills match the browser snapshot layout:[\s\S]*TextRect, subjectRect, and safeBottomY are verified:[\s\S]*Mobile safe zones are respected:[\s\S]*Motion bounds preserve readability:[\s\S]*Motion has a clear attention target:[\s\S]*Text transitions define entry, lock, emphasis, exit, and bridge:[\s\S]*Kinetic relay keyword chain is visible when relevant:[\s\S]*Action objects participate in transitions when relevant:[\s\S]*Video would lose meaning if reduced to screenshots:[\s\S]*Hold-frame verdict passed for hero frames:[\s\S]*## No-Go Gates[\s\S]*First-frame or hero-frame dominant visual object is not accidentally off-center or clipped:[\s\S]*Non-typography videos do not ship with placeholder-only image assets:[\s\S]*Pure-code exception is user-approved and stronger than using screenshots or generated plates:[\s\S]*Kinetic relay promos target 100 before final delivery:[\s\S]*Kinetic relay promos below 90 are blocked from final delivery:[\s\S]*Anti-PPT verdict:[\s\S]*## Layout Contract Revisions[\s\S]*## User Feedback Regression Review[\s\S]*## Recommended Next Edit[\s\S]*## Remaining Risks/,
    fields: [
